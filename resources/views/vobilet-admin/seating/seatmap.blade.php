						...
						<li class="seat @if($seat->reservationThisYear) @if($seat->reservationThisYear->status->id == 1) seat-reserved @elseif($seat->reservationThisYear->status->id == 2) seat-tempreserved @endif @if(Sentinel::getUser()->id == $seat->reservationThisYear->reservedfor->id && $seat->reservationThisYear->status->id == 1) seat-yours @endif @if(!is_null($seat->reservationThisYear->ticket)) @if(!is_null($seat->reservationThisYear->ticket->checkin)) checkedin @endif @endif @endif @if(Request::segment(4) == $seat->slug) active @endif" @if($seat->tickettype) style="background-color: #{{ $seat->tickettype->color }}" @endif>
							<p>
								@if(is_null($seat->reservationThisYear))
									<a href="{{ route('admin-seating-reservation-show', $seat->slug) }}" data-toggle="tooltip" title="{{ $seat->tickettype ? $seat->tickettype->name .': '. __('seating.map.available') : __('seating.map.unavailable') }}">{{ $seat->name }}</a>
								@elseif(Sentinel::getUser()->id == $seat->reservationThisYear->reservedfor->id && $seat->reservationThisYear->status->id == 1)
									<a href="{{ route('admin-seating-reservation-edit', $seat->reservationThisYear->id) }}" data-toggle="tooltip" title="{{ __('seating.map.you') }}">{{ $seat->name }}</a>
								@elseif($seat->reservationThisYear->status->id == 1)
									<a href="{{ route('admin-seating-reservation-edit', $seat->reservationThisYear->id) }}" data-toggle="tooltip" title="{{ __('seating.map.reservedfor') }}: {{ User::getUsernameAndFullnameByID($seat->reservationThisYear->reservedfor->id) }}">{{ $seat->name }}</a>
								@elseif($seat->reservationThisYear->status->id == 2)
									<a href="{{ route('admin-seating-reservation-edit', $seat->reservationThisYear->id) }}" data-toggle="tooltip" title="{{ __('seating.map.tempreserved') }}: {{ User::getUsernameAndFullnameByID($seat->reservationThisYear->reservedfor->id) }}">{{ $seat->name }}</a>
								@else
									{{ $seat->name }}
								@endif
							</p>
