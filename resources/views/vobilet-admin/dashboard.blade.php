		...
		<div class="card p-3">
			<div class="d-flex align-items-center">
				...
				<div>
					...
					<small class="text-muted">{{ User::where('last_activity', '')->orWhere('isAnonymized', '1')->count() }} deleted users</small>
				</div>
			</div>
		</div>
	</div>
			...
			<div class="card-body">
				<div class="row">
					<div class="col">
						...
						<div class="h3 m-0"><b>{{ User::where('last_activity', '<>', '')->where('isAnonymized', '0')->count() }} <small>/{{ User::all()->count() }}</small></b></div>
					</div>
					<div class="col-auto align-self-center">
						<div class="chart-circle chart-circle-xs" data-value="{{ sprintf("%.2f", User::where('last_activity', '<>', '')->orWhere('isAnonymized', '1')->orWhere('deleted_at', '<>', '')->count() / User::all()->count()) }}" data-thickness="6" data-color="#0061da"><canvas width="40" height="40"></canvas></div>
					</div>
				</div>
			</div>
		</div>
