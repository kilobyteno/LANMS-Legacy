<div class="seatmap">
	<ul>
		...
		<li class="entrance" id="entrance"><p><i class="fas fa-door-open"></i></p></li>
		...
		@foreach($rows as $row)
			...
			<li class="seat-row">
				<ul class="seat-row-{{$row->slug}}">
					@if($row->slug == 'a')
						<li class="seat kiosk" id="kiosk"><p><i class="fa fa-coffee"></i></p></li>
					@endif
					@foreach($row->seats as $seat)
						...
						<li class="seat @if($seat->reservationsThisYear()->count() > 0) 
							...
							@if(Sentinel::getUser()->id == $seat->reservationsThisYear()->first()->reservedfor->id) seat-yours @endif  @endif 
