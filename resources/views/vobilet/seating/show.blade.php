...
@section('title', $currentseat->name.' - '.__('seating.show.seat'))
@section('content')

<div class="container">
	<div class="page-header">
		<h4 class="page-title">{{ __('seating.show.seat') }} - {{ $currentseat->name }}</h4>
		<ol class="breadcrumb">
				...
				<div class="col-md-6">
					@if($currentseat->reservationsThisYear()->first())
						<div class="alert alert-info" role="alert">
							<i class="fas fa-info mr-2" aria-hidden="true"></i> {{ __('seating.show.reserved', ['type' => __('global.'.strtolower($currentseat->reservationsThisYear()->first()->status->slug))]) }}
						</div>
									...
									<div class="row">
										...
										<div class="col-md-8 pl-0">
											<div class="card-body cardbody">
												...
												<div class="cardprice">
													<span>{{ $currentseat->tickettype->price == 0 ? __('pages.tickets.free') : moneyFormat($currentseat->tickettype->price, 'NOK') }}</span>
												</div>
											</div>
					...
					@elseif(is_null($currentseat->tickettype) || !$currentseat->tickettype->active)
						<div class="alert alert-info" role="alert">
							<i class="fas fa-info mr-2" aria-hidden="true"></i>{{ __('seating.show.alert.cannotbereserved') }}
						</div>
					@elseif(Sentinel::getUser()->reservationsThisYear()->count() >= 5)
						<div class="alert alert-warning" role="alert">
							<i class="fas fa-exclamation mr-2" aria-hidden="true"></i> {!! __('seating.show.alert.reservationlimit') !!}
						</div>
					@elseif(!Setting::get('SEATING_OPEN'))
						<div class="alert alert-info" role="alert"> <i class="fas fa-info mr-2" aria-hidden="true"></i> {{ __('seating.show.alert.closed') }}</div>
					@else
						<div class="card card-item">
							<div class="card-body">
								<div class="border p-0">
						...
						<form method="post" action="{{ route('seating-reserve', $currentseat->slug) }}">
							...
							<div class="form-group">
								<label class="custom-switch">
									...
									<span class="custom-switch-description">{!! __('seating.show.agreement') !!}</span>
								</label>
							</div>
							<div class="form-group">
								<input type="hidden" name="_token" value="{{ csrf_token() }}">
								<button type="submit" class="btn btn-success" id="reserve" disabled=""><i class="fas fa-hand-paper"></i> {{ __('seating.show.button') }}</button>
							</div>
						</form>
					@endif
				</div>
