...
<div class="container">
    <div class="page-header">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item"><a href="{{ route('seating-checkin') }}">{{ trans('seating.checkin.title') }}</a></li>
				...
				<div class="card-body">
					...
					<div class="row text-center mt-5">
						<div class="col-12 col-lg-4">
							<h2><strong><small>{{ trans('global.type') }}:</small><br>{{ $ticket->reservation->seat->tickettype->name }}</strong></h2>
						</div>
						<div class="col-12 col-lg-4">
							@if(!$ticket->reservation->payment)
								<h2><strong><small>{{ trans('global.payment.paid') }}:</small><br><span class="text-danger">{{ trans('global.no') }}<br>{{ moneyFormat($ticket->reservation->seat->tickettype->price, Setting::get('MAIN_CURRENCY')) }}</span></strong></h2>
							@else
								<h2><strong><small>{{ trans('global.payment.paid') }}:</small><br><span class="text-success">{{ trans('global.yes') }}</span></strong></h2>
							@endif
						</div>
						<div class="col-12 col-lg-4">
							<h2><strong><small>{{ trans('pdf.ticket.yourseat') }}:</small><br>{{ $ticket->reservation->seat->name }}</strong></h2>
						</div>
					</div>
					<hr>
					<div class="mt-5">
