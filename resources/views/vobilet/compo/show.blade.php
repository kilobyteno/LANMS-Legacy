...
<div class="container">
	...
	<div class="row">
				...
				<div class="card-footer">
					@if(\Sentinel::check())
						...
						@elseif($compo->last_sign_up_at > \Carbon\Carbon::now() && \Sentinel::getUser()->composignups()->where('compo_id', $compo->id)->first())
							<a class="btn btn-sm btn-danger" href="{{ route('compo-signup-destroy', $compo->slug) }}"><i class="fas fa-user-slash"></i> {{ __('compo.signup.cancel') }}</a>
						@endif
					@endif
		...
		<div class="col-sm-12 col-md-6 col-lg-9 col-xl-9">
			<div class="card">
				<div class="card-header">
					<h3 class="card-title">{{ __('compo.type.'.$compo->type) }}</h3>
				</div>
				<div class="card-body d-flex flex-column">
					@if($compo->type == 1)
			...
			@if($compo->signups->count() > 0)
				<div class="card">
					<div class="card-header">
						<h3 class="card-title">{{ __('compo.attendees') }}</h3>
						<div class="card-options">
							@if(\Sentinel::check())
