...
@section('title', trans('user.addressbook.create.title'))
@section('content')

<div class="container">
	<div class="page-header">
		<h4 class="page-title">{{ trans('user.addressbook.create.title') }}</h4>
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">{{ trans('user.addressbook.create.title') }}</li>
		</ol>
	</div>
	<div class="row">
		<form class="col-md-12" role="form" method="post" action="{{ route('account-addressbook-store') }}"> 
			...
			<div class="card">
				...
				<div class="card-footer text-right">
					...
					<button type="submit" class="btn btn-success"><i class="fas fa-plus-square"></i> {{ trans('global.add') }} {{ trans('user.addressbook.address') }}</button>
				</div>
			</div>
		</form>
	</div>
