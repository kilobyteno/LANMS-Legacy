        ...
        <div class="col-md-12">
        	<div class="card">
        		<div class="card-header @if($invoice['status']=='draft') bg-info @elseif($invoice['status']=='paid') bg-success text-white @elseif($invoice['status']=='void' || $invoice['status']=='uncollectible') bg-danger text-white @elseif($invoice['status']=='open') bg-warning text-white @endif">
					<h3 class="card-title">{{ __('global.status') }}: {{ __('user.account.billing.invoice.status.'.$invoice['status']) }}</h3>
				</div>
				<div class="card-body">
					<div class="row ">
						<div class="col-lg-6">
								...
								@if($invoice['status']!='draft')
									...
									<tr class="d-print-none">
										<td colspan="5" class="text-right">
											@if(!$invoice['paid'])
												<a type="button" class="btn btn-success text-white" href="{{ route('account-billing-invoice-pay', $invoice['id']) }}"><i class="fas fa-shopping-cart"></i> {{ __('user.account.billing.invoice.payinvoice') }}</a>
											@endif
